...
Select Case Number
Case 1   'Form load
     Me.lblCaption.Caption = "Welcome to the LiveUpdate of " & AppName
...
Case 4
     ...
     Else
        Me.lblCaption.Caption = "Thank you for using LiveUpdate"
        Me.lblInfo.Caption = "LiveUpdate successfully !"
     End If
End Select
...
Dim nFlag As Long
    'MousePointer = vbHourglass
        frmLiveUpdate!lblEnd.Caption = "Internet connect..."
        DoEvents
    ...
    If hOpen <> 0 Then
      ...
      If hConnection <> 0 Then
         frmLiveUpdate!lblEnd.Caption = "You are connected on the server"
         FTPConnect = True
      Else
         FTPConnect = False
      End If
...
For x = 1 To 99
    ...
    If strFiles <> "" Then
       ...
       strLocalTime = RetFileDate(strPath)   'Date Heure du fichier Local
       If strFTPTime <= strLocalTime Then
          Me.lblEnd.Caption = "No update available for the files " & strFiles
       Else
          frmTransfert.Visible = True
          intNbrPasse = intNbrPasse + 1
          BlnResult = GetFiles(strFiles, strPath, SizeFile, 1)  'Si le transfert à réussi
...
If intNbrPasse = 0 Then
   Me.lblEnd.Caption = "Nom more recent update available at this moment !"
   Me.cmdBack.Enabled = False
   Me.cmdCancel.Enabled = False
   Me.cmdNext.Caption = "Terminer"
   SendFiles = False
Else
   If BlnResult = True Then
      Me.lblEnd.Caption = "Update finish " & intNbrPasse & " file(s) updated !"
      blnNewUpdate = True
      Me.cmdBack.Enabled = False
      Me.cmdCancel.Enabled = False
      Me.cmdNext.Caption = "Finish"
